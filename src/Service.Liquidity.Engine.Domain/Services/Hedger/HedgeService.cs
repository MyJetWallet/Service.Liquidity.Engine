    ...
    public class HedgeService : IHedgeService
    {
        ...
        private async Task HedgePositionAsync(PositionPortfolio positionPortfolio)
        {
            ...
            var instrumentSettings =
                _instrumentSettingsManager.GetHedgeInstrumentSettings(positionPortfolio.Symbol,
                    positionPortfolio.WalletId);
            ...
            try
            {
                ...
                if (wallet == null)
                {
                    _logger.LogError("Cannot hedge position {positionId}, because wallet '{marketName}' not found",
                        positionPortfolio.Id, instrumentSettings.WalletId);
                ...
                if (market == null)
                {
                    ...
                    _logger.LogError(
                        "Cannot hedge position {positionId}, because external market '{marketName}' not found",
                        positionPortfolio.Id, instrumentSettings.ExternalMarket);
                    activity?.SetStatus(Status.Error);
                    return;
                }
                ...
                _logger.LogInformation("Try to hedge position: {jsonText}",
                    JsonConvert.SerializeObject(positionPortfolio));

                new
                {
                    instrumentSettings.ExternalMarket, instrumentSettings.ExternalSymbol, hedgeSide, hedgeVolume,
                    hedgeReferenceId
                }.AddToActivityAsJsonTag("external-trade-request");
